static const unsigned int m_crc32table_256[256] =
{
...
unsigned int cal_crc32(unsigned int crc32, unsigned char *buf, unsigned int len)
{
	unsigned int i;
	unsigned int crc = crc32;
	for (i = 0; i < len; i++)
	{
		crc = m_crc32table_256[(crc ^ buf[i]) & 0xff] ^ (crc >> 8);
	}
	return crc;
}

unsigned int cal_crc32_with_salt(unsigned char *buf, unsigned int len, int salt)
{
	unsigned int i;
	unsigned int crc = (0xFFFFFFFF);
	unsigned char buf_salt = 0;
	for (i = 0; i < len; i++)
	{
		buf_salt = (buf[i] + salt) % 256;
		crc = m_crc32table_256[(crc ^ buf_salt) & 0xff] ^ (crc >> 8);
	}
	return crc;
}
